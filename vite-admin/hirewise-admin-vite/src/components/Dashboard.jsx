...
const getPositionFilterOptions = () => {
  ...
  <div className="p-4 space-y-4">
        ...
        {/* Shortlisted Candidates Section - WITH Schedule Column */}
        <div className="bg-white rounded-lg shadow-sm p-4 mx-4 mt-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-gray-900">
              Shortlisted Candidates ({loading ? 0 : sortedCandidates.length})
            </h2>
          </div>
                    ...
                    <tr key={candidate.id || candidate.rank || index} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-2">
                        {(() => {
                          ...
                          if (confirmState?.status === 'ACCEPTED') {
                            return (
                              <button
                                ...
                                className="px-3 py-1 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors font-medium"
                              >
                                Schedule
                              </button>
                            );
                          ...
                          // Default: Show Send Confirmation button
                          return (
                            <button
                              onClick={() => sendConfirmationEmail(candidate)}
                              disabled={sendingConfirmation[candidate.id]}
                              className="px-3 py-1 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition-colors font-medium disabled:opacity-50"
                            >
                              {sendingConfirmation[candidate.id] ? 'Sending...' : 'Send Confirmation'}
                            </button>
                          );
                        })()}
                      </td>
